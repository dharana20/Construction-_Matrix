...
export default function Header({ onExportPDF, onOpenGuide, onReset }) {
  return (
    <header className="glass-panel p-4 sm:p-5 mb-6 sticky top-2 z-40 border-b border-amber-500/20 shadow-2xl backdrop-blur-xl">
      <div className="w-full max-w-[1800px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        
        {/* Logo & Main Title */}
        <div className="flex items-center space-x-3.5 w-full md:w-auto justify-between md:justify-start">
          <div className="flex items-center space-x-3.5">
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-amber-500 via-orange-600 to-amber-700 flex items-center justify-center shadow-lg shadow-orange-500/30 ring-2 ring-amber-400/40">
        ...
        <div className="flex items-center flex-wrap gap-3 w-full md:w-auto justify-end">
          <button
            onClick={onOpenGuide}
            className="flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-semibold rounded-xl bg-slate-900/80 hover:bg-slate-800 text-amber-300 border border-amber-500/30 transition-all shadow-sm"
          >
          ...
          <button
            onClick={onReset}
            className="flex items-center justify-center gap-1.5 px-3.5 py-2.5 text-xs font-semibold rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 border border-slate-700 transition-all"
